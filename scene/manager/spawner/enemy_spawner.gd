...
func _ready() -> void:
	enemy_pool.add_mob(mushroom_scene, 70)
...
func on_difficulty_increased(difficulty_level: int):
	var new_spawn_time = max(minimum_spawn_time, base_spawn_time - (difficulty_level * difficulty_multiplier))
	timer.wait_time = new_spawn_time
	if difficulty_level == 4:
		enemy_pool.add_mob(goblin_scene, 70)
	elif difficulty_level == 10:
		enemy_pool.add_mob(wolf_scene, 10)
	elif difficulty_level == 20:
		enemy_pool.add_mob(mini_boss_scene, 5)
